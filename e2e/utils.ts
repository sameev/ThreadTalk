...
export async function signUp(
  ...
  const welcomeNotice = page.locator("h2", { hasText: "Welcome to Samee's Project Message Board!" });
...
export async function login(
  ...
  const passwordInput = page.locator('input[name="password"]');
  await passwordInput.fill(password);
  const signUpSubmitButton = page.locator("button.supabase-auth-ui_ui-button");
